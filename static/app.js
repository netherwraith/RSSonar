...
$('#opml-export').addEventListener('click', () => withAdmin(async () => {
  const response = await fetch('/api/feeds/export', {
    headers: { 'X-Admin-Token': token }, cache: 'no-store'
  });
  if (!response.ok) {
    let message = t('exportFailed');
    try { message = translateApiError((await response.json()).error || message); } catch (_) { /* Keep fallback. */ }
  ...
  link.href = fileUrl;
  link.download = 'rssonar-feeds.opml';
